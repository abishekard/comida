...
{
    public function currentSales(Request $request)
    {
        ...
        $currentWeekCodData = DB::table('customer_order_table')->where('partner_id',$request->partner_id)->where('payment_method', 'cod')
            ->where('week', $weekNow)->where('month', $monthNow)->where('year', $yearNow)->pluck('total_price');
        $currentWeekOnlineData = DB::table('customer_order_table')->where('partner_id',$request->partner_id)->where('payment_method', 'online')
            ->where('week', $weekNow)->where('month', $monthNow)->where('year',  $yearNow)->pluck('total_price');
        ...
        $currentMonthCodData = DB::table('customer_order_table')->where('partner_id',$request->partner_id)->where('payment_method', 'cod')
            ->where('month', $monthNow)->where('year',  $yearNow)->pluck('total_price');
        $currentMonthOnlineData = DB::table('customer_order_table')->where('partner_id',$request->partner_id)->where('payment_method', 'online')
            ->where('month', $monthNow)->where('year', $yearNow)->pluck('total_price');
...
*/
        ...
        $todayCodData = DB::table('customer_order_table')->where('partner_id',$request->partner_id)->where('payment_method', 'cod')
            ->where('date',$today)->pluck('total_price');
        $todayOnlineData = DB::table('customer_order_table')->where('partner_id',$request->partner_id)->where('payment_method', 'online')
            ->where('date',$today)->pluck('total_price');
